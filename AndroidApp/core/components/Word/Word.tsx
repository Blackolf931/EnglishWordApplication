    ...
    useEffect(() => {
        word.translatedWords.map((x, indexRightAnswer) => {
            if (x == word.translatedWord) {
                setRightButtonIndex(indexRightAnswer),
                    console.log(indexRightAnswer)
            }
        });
    }, [word])
            ...
            <FlatGrid
                ...
                renderItem={({ item, index }) => (
                    <Button
                        ...
                        contentStyle={
                            state.buttonKey === index && state.isRightAnswer ? styles.correctAnswer : state.buttonKey === index && !state.isRightAnswer ? styles.incorrectAnswer : rightButtonIndex === index && state.disabled === true ? styles.correctAnswer : styles.buttons}
                    >
                        {item}
                    </Button>
                )}
            />
            <Button onPress={() => { setIndex(numberQuestion + 1) }}> Next Word</Button>
        </View>
    )
}